...
namespace TopDownShooterSpike.Simulation.Objects
{
    public sealed class Map : Actor
    {
        ...
        public Map(IActorService service, ITileProvider provider) : this(service, provider, 64, 64)
        {
            this.FillRect(0, 0, Width-1, Height-1, false, Tile.Grass);
        }

        public Map(IActorService service, ITileProvider provider, int width, int height) : base(service)
        {
        ...
        public static Map FromString(IActorManager manager, IActorService service, ITileProvider provider, string mapData)
        {
            ...
            var result = new Map(service, provider, width, height);
